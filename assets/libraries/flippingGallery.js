...
!(function($) {
  ...
  $.fn.flipping_gallery = function(options) {
    var settings = $.extend({}, defaults, options), el = $(this),
        total = el.find(settings.selector).length, space = 0,
        scale = 0.5 / total, opacity = 1 / total, lastAnimation = 0,
        quietPeriod = 500;

    if (total > settings.showMaximum)
      opacity = 1 / settings.showMaximum;
    el.addClass("fg-body");
      ...
      if (!el.hasClass("animating")) {
        ...
        card.one(
            ...
            function(e) {
              var save_card =
                  card.removeClass(
                          "animate active fg-flipping [class^='fg-count-'] " +
    ...
    $.fn.flipBackward = function() {
      if (!el.hasClass("animating")) {
        ...
        el.prepend(
            new_prev_card.attr("style", "")
                .css({
                  opacity : "0",
                  "z-index" : "99",
                })
                .hide()
                .addClass("active fg-flipping " + settings.flipDirection));
        el.find("> .fg-card.active")
            .addClass("animate")
            .show()
            .removeClass("fg-flipping")
            .css("opacity", "1");
        space = 0;
        el.realignCards();
        el.find("> .fg-card")
            .one(
                "webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend",
                function(e) { el.removeClass("animating"); });
      }
    };
